...
function createFilterControls() {
  ...
  // ✅ TRADUIT : labels et options du filtre
  filterContainer.innerHTML = `
        <div style="flex: 2; min-width: 200px;">
            <label style="display: block; margin-bottom: 8px; font-weight: 600;"><i class="fas fa-search"></i> ${t("filter.search")}</label>
            <input type="text" id="search-filter" placeholder="${t("filter.search_placeholder")}" style="width:100%; padding:10px 12px; border:1px solid #ced4da; border-radius:6px;">
        </div>
        <div style="min-width: 160px;">
            <label style="display: block; margin-bottom: 8px; font-weight: 600;"><i class="fas fa-filter"></i> ${t("filter.role")}</label>
            <select id="role-filter" style="width:100%; padding:10px 12px; border:1px solid #ced4da; border-radius:6px;">
                ...
                <option value="0">${t("role.superadmin")}</option>
                <option value="1">${t("role.admin")}</option>
                <option value="2">${t("role.user")}</option>
                <option value="3">${t("role.logisticien")}</option>
                <option value="4">${t("role.comptable")}</option>
            </select>
        </div>
        <div style="min-width: 140px;">
            <label style="display: block; margin-bottom: 8px; font-weight: 600;"><i class="fas fa-chart-line"></i> ${t("filter.status")}</label>
        ...
        <div style="min-width: 140px;">
            <label style="display: block; margin-bottom: 8px; font-weight: 600;"><i class="fas fa-table"></i> ${t("filter.rows_per_page")}</label>
            <select id="rows-per-page-top" style="width:100%; padding:10px 12px; border:1px solid #ced4da; border-radius:6px;">
                <option value="5">${t("filter.rows", { n: 5 })}</option>
                <option value="10" selected>${t("filter.rows", { n: 10 })}</option>
                <option value="20">${t("filter.rows", { n: 20 })}</option>
                <option value="50">${t("filter.rows", { n: 50 })}</option>
                <option value="100">${t("filter.rows", { n: 100 })}</option>
        ...
        <div>
            <button id="reset-filters" style="padding:10px 24px; background:#6c757d; color:white; border:none; border-radius:6px; cursor:pointer;">
                <i class="fas fa-undo-alt"></i> ${t("filter.reset")}
            </button>
        </div>
    `;
  ...
  document
    ...
    ?.addEventListener("change", applyFilters);
  document
    .getElementById("rows-per-page-top")
...
function applyFilters() {
  var searchTerm =
    document.getElementById("search-filter")?.value.toLowerCase().trim() || "";
...
function createPaginationControls(totalPages) {
  ...
  container.style.cssText =
    "margin:20px 0; display:flex; justify-content:center; gap:5px; flex-wrap:wrap;";
  ...
  var createBtn = function (text, onClick, disabled, isDots) {
    ...
    if (isDots) {
      btn.style.cssText =
        "padding:8px 12px; border:none; background:transparent; color:#6c757d; cursor:default;";
      return btn;
    }
    var isActive = text == currentPage && !isNaN(text);
    btn.style.cssText =
      "padding:8px 14px; border:1px solid " +
      (isActive ? "#007bff" : "#dee2e6") +
      ";" +
      "background:" +
      (isActive ? "#007bff" : disabled ? "#e9ecef" : "white") +
      ";" +
      "color:" +
      (isActive ? "white" : disabled ? "#6c757d" : "#007bff") +
      ";" +
      "cursor:" +
  ...
  if (end < totalPages) {
    if (end < totalPages - 1)
      container.appendChild(createBtn("...", null, true, true));
    (function (tp) {
      container.appendChild(
        createBtn(tp, function () {
          goToPage(tp);
        }),
      );
    })(totalPages);
...
function updateCounter() {
  ...
  if (!counter) {
    ...
    counter.style.cssText =
      "margin:15px 0 0; padding:10px 15px; text-align:center; font-size:14px; background:#f8f9fa; border-radius:6px; border:1px solid #e9ecef;";
  ...
  counter.innerHTML =
    ...
    t("message.display_range", {
      start: filteredUsers.length === 0 ? "0" : start,
      end: end,
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (
        ...
        (submitter.classList.contains("btn-success") ||
          ...
          (submitter.getAttribute("onclick") &&
...
function ensureButtonsHaveTypeButton() {
  var buttons = document.querySelectorAll(
    ".action-buttons button, .dash-table button, .modal-footer button",
  );
